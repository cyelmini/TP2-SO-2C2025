...
static uint64_t ps() {
	...
	for (uint16_t i = 0; i < qty; i++) {
		printf("PID:%d  NAME:%s  STATUS:%d  PRIO:%d\n", (int) list[i].pid, list[i].name ? list[i].name : "(null)",
			   (int) list[i].status, (int) list[i].priority);
...
static uint64_t wc() {
	...
	// Hacer varios yields para asegurar que el printf se complete
	for (int i = 0; i < 100; i++) {
		sys_yield();
	}
...
pid_t handle_filter(char **argv, int argc, int ground, int stdin, int stdout) {
	...
	uint8_t priority = 1;
	pid_t pid =
		(pid_t) sys_createProcess((uint64_t) filter, argv, argc > 0 ? argc : 1, priority, (char) (!ground), fds);
	return ground ? pid : 0;
}
...
pid_t handle_test_processes(char **argv, int argc, int ground, int stdin, int stdout) {
	...
	pid_t pid =
		sys_createProcess((uint64_t) run_test_processes, argv, argc > 0 ? argc : 1, priority, (char) (!ground), fds);
	return ground ? pid : 0;
}
